...
class HMM():
    ...
    def viterbi(self, sequence):
        ...
        #may need to change datatype
        v_dist = np.zeros((seq_len+1,self.num_states),dtype=float)
        v_path = []
        #store probability p as 10 base log of x. 10^x = p
        maxState = 0
        maxVal = float("-inf")
        for x in range(self.num_states):
        	tempVal = self.prior[x] + math.log10(self.emission[x][sequence[0]])
        	if tempVal > maxVal:
        		maxVal = tempVal
        		maxState = x
        v_dist[1] = maxVal
        v_path.append(maxState)
        assert(math.pow(10,v_dist[1]) == self.prior[maxState]*self.emission[maxState][sequence[0]])
        for evidence in range(1,seq_len):
        	maxVal = float("-inf")
        	for state in range(self.num_states):
        		tempVal = v_dist[evidence] + math.log10(self.emission[state][sequence[evidence]]) + math.log10(self.transition[maxState][state])
        		if 
