...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if coor in Corners:
            if coor not in visitedCorners:
                visitedCorners.append(coor) 
            return numCorners == len(visitedCorners)
        return False
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    def minCorner(m_coor,m_corners):
        """finds (min distance,index) for given list of corners and start point coor"""
        distances = []
        numCorners = len(m_corners)
        for x in range(numCorners):
            distance = util.manhattanDistance(m_coor,m_corners[x])
    ...
    for x in corners:
        if x not in visitedCorners:
            unvisitedCorners.append(x)
    numUnvisited = len(unvisitedCorners)
    for x in range(numUnvisited):
        minDistance = minCorner(coor,unvisitedCorners)
        distance += minDistance[0]
        coor = unvisitedCorners.pop(minDistance[1])
    return distance
