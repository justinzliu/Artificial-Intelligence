...
class DenseLayer(BaseLayer):
    ...
    def compute_gradient(self):
        ...
        self.dw = np.dot(self.get_input_data().transpose(),self._output_error_gradient)
        oneVector = [np.ones(self._output_error_gradient.shape[0],)]

        self.db = np.dot(oneVector,self._output_error_gradient)

        self._input_error_gradient = np.dot(self._output_error_gradient,self.w.transpose())
    ...
    def update_weights(self, learning_rate):
        ...
        gradient_w = self.dw*learning_rate
        gradient_b = self.db*learning_rate
        self.w = np.subtract(self.w,gradient_w)
        self.b = np.subtract(self.b,gradient_b)
